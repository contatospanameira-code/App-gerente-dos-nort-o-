...
async function rotaAnthropicAnalisar({ imgBase64, imgMime, valor, nomeFuncionario }, env, cors, ip) {
  ...
  const resp = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 600, messages: [{ role: 'user', content: [{ type: 'image', source: { type: 'base64', media_type: mimeSeguro, data: imgBase64 } }, { type: 'text', text: prompt }] }] })
  });
  if (!resp.ok) {
