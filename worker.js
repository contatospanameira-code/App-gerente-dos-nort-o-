...
async function rotaAnthropicAnalisar({ imgBase64, imgMime, imgUrl, valor, nomeFuncionario }, env, cors, ip) {
  ...
  if(!base64Final && imgUrl){
    try{
      ...
      if(!r.ok) return json({ aprovado: true, erro: 'Não foi possível buscar a imagem da URL.' }, 200, cors);
      ...
      mimeSeguro  = ['image/jpeg','image/png','image/webp','image/gif'].includes(contentType) ? contentType : 'image/jpeg';
    }catch(e){
      return json({ aprovado: true, erro: 'Erro ao buscar imagem: ' + e.message }, 200, cors);
    }
  }

  if (!base64Final || base64Final.startsWith('pdf:')) return json({ aprovado: true, erro: 'PDF aprovado manualmente' }, 200, cors);
  ...
  if(imgMime) mimeSeguro = mimesPermitidos.includes(imgMime) ? imgMime : mimeSeguro;
